function [value, mult] = recourse(lambda, yields, x, s, alpha)
...
cons = [cons, e_H <=6000];
cons = [cons, R == (170 * w_W + 150 * w_C + 36 * e_H + 10 * e_L - 238 * y_W - 210*y_C)]; %- Costs*dummy)];
cons = [cons, [w_W; w_C; e_H; e_L; y_W; y_C] >=0 ];
cons = [cons, z >= 0];
cons = [cons, z >= dummy(4) - R - Costs * dummy(1:3)];
